...
const moduleName = "battlezoo-ancestries-dads"
...
const symlinkPath = path.resolve(dataPath, "modules", moduleName);
...
if (symlinkStats) {
    ...
    const proceed = (
        await prompts({
            ...
            name: "value",
            initial: false,
            message: `A "${moduleName}" ${atPath} already exists in the "modules" subfolder. Replace with new symlink?`,
        })
    ).value;
    if (!proceed) {
